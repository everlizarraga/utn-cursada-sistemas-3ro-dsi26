...
👀 *Con Lombok existe el atajo `@Slf4j` sobre la clase: te genera el campo `log` solo (adiós a la línea del `LoggerFactory`). Válido y muy común — en el advice quedó explícito para que vieras la pieza; en tu TP, `@Slf4j` es lo esperable.*
